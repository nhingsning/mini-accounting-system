@extends('layouts.app')
@section('title', ($po->number ?? ('PO #'.$po->id)))
    ...
    <div class="topbar">
      ...
      <h2 class="m-0">
        ...
        <span class="badge ms-2
          {{ (strtolower($po->status ?? '')==='approved') ? 'text-bg-success' :
             ((strtolower($po->status ?? '')==='cancelled') ? 'text-bg-danger' : 'text-bg-warning') }}">
          {{ ucfirst($po->status ?? 'Draft') }}
        </span>
      </h2>
      <div class="ms-auto d-flex gap-2">
        <a href="{{ route('po.index') }}" class="btn btn-light"><i class="bi bi-arrow-left"></i> Back</a>
        @if(Route::has('po.edit'))
          <a href="{{ route('po.edit',$po->id) }}" class="btn btn-brand"><i class="bi bi-pencil"></i> Edit</a>
        @endif
      ...
      <div class="row g-3">
            ...
            <div class="panel-body">
              <div class="row g-3">
                <div class="col-md-6">
                  ...
                  <div class="fw-semibold">{{ $po->customer_name ?? '—' }}</div>
                </div>
                <div class="col-md-3">
                  <div class="mini">Issue Date</div>
                  <div class="fw-semibold">
                    {{ \Carbon\Carbon::parse($po->issue_date ?? $po->created_at)->format('M d, Y') }}
                  </div>
                </div>
                <div class="col-md-3">
                  <div class="mini">Due Date</div>
                  <div class="fw-semibold">
                    {{ $po->due_date ? \Carbon\Carbon::parse($po->due_date)->format('M d, Y') : '—' }}
                  </div>
                </div>
              </div>
        ...
        <div class="col-12 col-xl-4">
          <div class="panel">
            ...
            <div class="panel-body">
              @php
                $subtotal  = round($po->subtotal ?? ($po->items? $po->items->sum('line_total'):0), 2);
                $discRate  = (float)($po->discount_rate ?? 0);
                $taxRate   = (float)($po->tax_rate ?? 0);
                $taxValue  = round($po->tax ?? ($subtotal*(1-($discRate/100))*($taxRate/100)), 2);
                $total     = round($po->total ?? ($subtotal*(1-($discRate/100)) + $taxValue), 2);
              @endphp
          ...
          <div class="mini mt-2">Last updated: {{ $po->updated_at?->format('M d, Y H:i') }}</div>
        </div>
      </div>
    </div>
  </main>
